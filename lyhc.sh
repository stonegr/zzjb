...
check_system() {
    if [[ ! -z "$(cat /etc/issue | grep -iE "debian")" ]]; then
        ...
        if [[ $? -ne 0 ]]; then apt-get install traceroute -y; fi
        mtr -v >/dev/null 2>&1
        if [[ $? -ne 0 ]]; then apt-get install mtr -y; fi
...
directory() {
    [[ ! -d /root/tstrace ]] && mkdir -p /root/tstrace
    cd /root/tstrace
}
install() {
    [[ ! -e /root/tstrace/besttrace ]] && wget -O besttrace https://github.com/stonegr/zzjb/raw/main/besttrace
    chmod -R +x /root/tstrace
}
...
test_single() {
    ...
    ./besttrace -q1 -g cn ${ip} | tee -a -i /root/tstrace/tstrace.log 2>/dev/null
...
if [[ "${function}" == "1" ]]; then
    ...
    test_all | tee -a -i /root/tstrace/tstrace.log 2>/dev/null
else
    test_single
fi
