...
setupService() {
    # wget --no-check-certificate -qO /usr/local/bin/dnat.sh https://raw.githubusercontent.com/stonegr/zzjb/main/dnat.sh || {
...
EOF
    sed -i 's/^\ \ \ \ //g' /usr/local/bin/dnat.sh
...
EOF
    sed -i 's/^\ \ \ \ //g' /lib/systemd/system/dnat.service
...
rmIptablesNat() {
    ...
    for cell in ${arr1[@]}; do                  # cell= 1:tcp:to:8.8.8.8:543
...
select todo in 增加到域名的转发 删除到域名的转发 增加到IP的转发 删除到IP的转发 列出所有到域名的转发 查看iptables转发规则; do
    case $todo in
    增加到域名的转发)
        addDnat
        #break
